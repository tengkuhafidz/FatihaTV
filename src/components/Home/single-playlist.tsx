...
const SinglePlaylist: React.FunctionComponent<Props> = ({playlist, isPlaylistPinnedLocally, handleTagFilterClick}) => {
    ...
    const renderTags = () => {
        return tagsArray.map(tag => (
            <span 
                key={tag} 
                className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 hover:bg-teal-500 hover:text-white" onClick={(e) => handleTagFilterClick(e, tag)} 
            >
                #{tag}
            </span>
        ))
    ...
    return (
        <div 
            className={`rounded overflow-hidden shadow-lg hover:shadow-2xl bg-white align-center cursor-pointer  ${ isPlaylistPinnedLocally ? "border-teal-500 border-4" : "" }`} 
